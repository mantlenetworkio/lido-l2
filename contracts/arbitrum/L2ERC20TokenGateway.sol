...
contract L2ERC20TokenGateway is InterchainERC20TokenGateway, L2CrossDomainEnabled, IL2TokenGateway {
    ...
    constructor(address arbSys_, address router_, address counterpartGateway_, address l1Token_, address l2Token_)
    ...
    ) external whenWithdrawalsEnabled onlySupportedL1Token(l1Token_) returns (bytes memory res) {
        address from = L2OutboundDataParser.decode(router, data_);

        IERC20Bridged(l2Token).bridgeBurn(from, amount_);

        uint256 id =
            sendCrossDomainMessage(from, counterpartGateway, getOutboundCalldata(l1Token_, from, to_, amount_, ""));
    ...
    function finalizeInboundTransfer(address l1Token_, address from_, address to_, uint256 amount_, bytes calldata)
        external
        whenDepositsEnabled
