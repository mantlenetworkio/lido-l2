...
library L1OutboundDataParser {
    ...
    function decode(address router_, bytes memory data_) internal view returns (address, uint256) {
    ...
    function _parseSubmissionCostData(bytes memory data_) private pure returns (uint256) {
        (uint256 maxSubmissionCost, bytes memory extraData) = abi.decode(data_, (uint256, bytes));
